...
#    sed -i -e '1{x;s/^/first/;x;}1,/AllowOverride None/{x;/first/s///;x;s/AllowOverride None/AllowOverride All/;}'  /etc/apache2/httpd.conf ; \
#    sed -i "s/;extension=pdo_mysql/extension=pdo_mysql.so/" /etc/php7/php.ini ; \
#    sed -i "s/;extension=pdo_odbc/extension=pdo_odbc.so/" /etc/php7/php.ini ; \
#    sed -i "s/;extension=pdo_pgsql/extension=pdo_pgsql.so/" /etc/php7/php.ini ; \
#    sed -i "s/;extension=pdo_sqlite/extension=pdo_sqlite.so/" /etc/php7/php.ini
